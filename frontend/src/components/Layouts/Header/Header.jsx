...
const Header = () => {
    ...
    return (
        <header className="bg-black fixed top-0 py-2.5 w-full z-10">
            ...
            <div
                style={{ width: "100%" }}
                className="w-full sm:w-9/12 px-1 sm:px-4 m-auto flex justify-around items-center relative"
            >
                ...
                <div className="flex w-2/4 items-center  mx-auto">
                    {user?.role !== "admin" ? <Searchbar /> : null}
                </div>
                {/* <!-- logo & search container --> */}
                ...
                <div className="flex items-center justify-between ml-1 sm:ml-0 gap-0.5 sm:gap-7 relative">
                    ...
                    {isAuthenticated === false ? (
                        <Link
                            to="/login"
                            className="px-3 sm:px-9 py-0.5  bg-white border font-medium rounded-sm cursor-pointer"
                        >
                        ...
                        >
                            <AccountCircleIcon></AccountCircleIcon>
                            {user.name && user.name.split(" ").slice(-1)}
                            <span>
